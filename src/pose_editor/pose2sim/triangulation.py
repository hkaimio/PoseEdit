...
def make_trc(config_dict, Q, keypoints_names, f_range, id_person=-1):
    ...
    if frame_rate == "auto":
        try:
            ...
            logging.warning("Cannot read video. Frame rate will be set to 60 fps.")
            frame_rate = 30
...
def recap_triangulate(
    ...
    cal_keys = [
        c
        for c in calib.keys()
        if c not in ["metadata", "capture_volume", "charuco", "checkerboard"] and isinstance(calib[c], dict)
    ]
    ...
    for n in range(nb_persons_to_detect):
        ...
        for idx, name in enumerate(keypoints_names):
            ...
            if show_interp_indices:
                if interpolation_kind != "none":
                    ...
                    logging.info("  No frames were interpolated because 'interpolation_kind' was set to none. ")
        ...
        cam_excluded_count[n] = {i: v for i, v in zip(cam_names, cam_excluded_count[n].values(), strict=False)}
...
def extract_files_frame_f(json_tracked_files_f, keypoints_ids, nb_persons_to_detect):
    ...
    for n in range(nb_persons_to_detect):
        for cam_nb in range(n_cams):
            ...
            try:
                with open(json_tracked_files_f[cam_nb]) as json_f:
                    js = json.load(json_f)
...
def triangulate_all(config_dict):
    ...
    try:
        ...
        raise Exception("No .toml calibration direcctory found.")
    try:
        calib_files = glob.glob(os.path.join(calib_dir, "*.toml"))
        calib_file = max(calib_files, key=os.path.getctime)  # lastly created calibration file
    except:
    ...
    try:  # from skeletons.py
        if pose_model.upper() == "BODY_WITH_FEET":
            pose_model = "HALPE_26"
        elif pose_model.upper() == "WHOLE_BODY_WRIST":
            pose_model = "COCO_133_WRIST"
        elif pose_model.upper() == "WHOLE_BODY":
            pose_model = "COCO_133"
        elif pose_model.upper() == "BODY":
            pose_model = "COCO_17"
    ...
    try:
        ...
        keypoints_idx_swapped = [
            keypoints_names.index(keypoint_name_swapped) for keypoint_name_swapped in keypoints_names_swapped
    ...
    try:
        json_files_names = [
            fnmatch.filter(os.listdir(os.path.join(poseTracked_dir, js_dir)), "*.json") for js_dir in json_dirs_names
        ]
        pose_dir = poseTracked_dir
    except:
        try:
            json_files_names = [
                fnmatch.filter(os.listdir(os.path.join(poseSync_dir, js_dir)), "*.json") for js_dir in json_dirs_names
            ]
            pose_dir = poseSync_dir
        except:
            try:
                json_files_names = [
                    fnmatch.filter(os.listdir(os.path.join(pose_dir, js_dir)), "*.json") for js_dir in json_dirs_names
                ]
            except:
                raise Exception(
    ...
    if n_cams != len(P):
        raise Exception(
            f"Error: The number of cameras is not consistent: Found {len(P)} cameras in the calibration file, and {n_cams} cameras based on the number of pose folders."
        )
    ...
    if multi_person:
        nb_persons_to_detect = max(
            max(
                count_persons_in_json(os.path.join(pose_dir, json_dirs_names[c], json_fname))
    ...
    for f in tqdm(range(*f_range)):
        ...
        json_files_names_f = [
            [j for j in json_files_names[c] if int(re.split(r"(\d+)", j)[-2]) == f] for c in range(n_cams)
        ]
        ...
        if undistort_points:
            for n in range(nb_persons_to_detect):
                points = [
                    np.array(tuple(zip(x_files[n][i], y_files[n][i], strict=False))).reshape(-1, 1, 2).astype("float32")
                    for i in range(n_cams)
                ]
                undistorted_points = [
                    cv2.undistortPoints(
                        points[i], calib_params["K"][i], calib_params["dist"][i], None, calib_params["optim_K"][i]
                    )
                    for i in range(n_cams)
                ]
    ...
    Q_tot = [list(tpl) for tpl in zip(*it.zip_longest(*Q_tot, fillvalue=[np.nan] * keypoints_nb * 3), strict=False)]
    error_tot = [list(tpl) for tpl in zip(*it.zip_longest(*error_tot, fillvalue=[np.nan] * keypoints_nb * 3), strict=False)]
    nb_cams_excluded_tot = [
        list(tpl) for tpl in zip(*it.zip_longest(*nb_cams_excluded_tot, fillvalue=[np.nan] * keypoints_nb * 3), strict=False)
    ]
    id_excluded_cams_tot = [
        list(tpl) for tpl in zip(*it.zip_longest(*id_excluded_cams_tot, fillvalue=[np.nan] * keypoints_nb * 3), strict=False)
    ]

    # dataframes for each person
    Q_tot = [
        pd.DataFrame([Q_tot_f[n] for Q_tot_f in Q_tot], index=range(*f_range)) for n in range(nb_persons_to_detect)
    ]
    error_tot = [
        pd.DataFrame([error_tot_f[n] for error_tot_f in error_tot], index=range(*f_range))
        for n in range(nb_persons_to_detect)
    ]
    nb_cams_excluded_tot = [
        pd.DataFrame(
            [nb_cams_excluded_tot_f[n] for nb_cams_excluded_tot_f in nb_cams_excluded_tot], index=range(*f_range)
        )
        for n in range(nb_persons_to_detect)
    ]
    id_excluded_cams_tot = [
        pd.DataFrame(
            [id_excluded_cams_tot_f[n] for id_excluded_cams_tot_f in id_excluded_cams_tot], index=range(*f_range)
        )
        for n in range(nb_persons_to_detect)
    ]
    ...
    f_range_trimmed = [
        indices_of_first_last_non_nan_chunks(
            err["mean"], min_chunk_size=interp_gap_smaller_than, chunk_choice_method=sections_to_keep
        )
        for err in error_tot
    ]
    ...
    nb_cams_excluded_tot = [
        nb_cams_excluded_tot[n] for n in range(len(nb_cams_excluded_tot)) if n not in deleted_person_id
    ]
    id_excluded_cams_tot = [
        id_excluded_cams_tot[n] for n in range(len(id_excluded_cams_tot)) if n not in deleted_person_id
    ]
    ...
    logging.info(
        "Trimming "
        + ", ".join([f"Participant {n + 1} around frames {f_range_trimmed[n]}" for n in range(nb_persons_to_detect)])
        + "."
    )
    Q_tot = [Q_tot[n].loc[f_range_trimmed[n][0] : f_range_trimmed[n][1] - 1] for n in range(nb_persons_to_detect)]
    error_tot = [
        error_tot[n].loc[f_range_trimmed[n][0] : f_range_trimmed[n][1] - 1] for n in range(nb_persons_to_detect)
    ]
    nb_cams_excluded_tot = [
        nb_cams_excluded_tot[n].loc[f_range_trimmed[n][0] : f_range_trimmed[n][1] - 1]
        for n in range(nb_persons_to_detect)
    ]
    id_excluded_cams_tot = [
        id_excluded_cams_tot[n].loc[f_range_trimmed[n][0] : f_range_trimmed[n][1] - 1]
        for n in range(nb_persons_to_detect)
    ]
    zero_nan_frames = [
        np.where(Q_tot[n].loc[:, ::3].T.eq(0) | ~np.isfinite(Q_tot[n].iloc[:, ::3].T))
        for n in range(nb_persons_to_detect)
    ]
    zero_nan_frames_per_kpt = [
        [zero_nan_frames[n][1][np.where(zero_nan_frames[n][0] == k)[0]] for k in range(keypoints_nb)]
        for n in range(nb_persons_to_detect)
    ]
    zero_nan_frames_per_kpt = [
        [z[(f_range_trimmed[n][0] < z) & (f_range_trimmed[n][1] > z)] for z in zero_nan_frames_per_kpt[n]]
        for n in range(nb_persons_to_detect)
    ]
        ...
        for n in range(nb_persons_to_detect):
            try:
                Q_tot[n] = Q_tot[n].apply(
                    interpolate_zeros_nans, axis=0, args=[interp_gap_smaller_than, interpolation_kind]
                )
            except:
                logging.info(
    ...
    # Create TRC file
    trc_paths = [
        make_trc(config_dict, Q_tot[n], keypoints_names, f_range_trimmed[n], id_person=n) for n in range(len(Q_tot))
    ]
    if make_c3d:
    ...
    id_excluded_cams_tot = [
        np.hstack(np.hstack(np.array(id_excluded_cams_tot[n]))) for n in range(nb_persons_to_detect)
    ]
    cam_excluded_count = [dict(Counter(k)) for k in id_excluded_cams_tot]
    [
        cam_excluded_count[n].update((x, y / frame_nb / keypoints_nb) for x, y in cam_excluded_count[n].items())
        for n in range(nb_persons_to_detect)
    ]
    ...
    if show_interp_indices:
        gaps = [
            [np.where(np.diff(zero_nan_frames_per_kpt[n][k]) > 1)[0] + 1 for k in range(keypoints_nb)]
            for n in range(nb_persons_to_detect)
        ]
        sequences = [
            [np.split(zero_nan_frames_per_kpt[n][k], gaps[n][k]) for k in range(keypoints_nb)]
            for n in range(nb_persons_to_detect)
        ]
        interp_frames = [
            [
                [f"{seq[0]}:{seq[-1]}" for seq in seq_kpt if len(seq) <= interp_gap_smaller_than and len(seq) > 0]
        ...
        non_interp_frames = [
            [
                [f"{seq[0]}:{seq[-1]}" for seq in seq_kpt if len(seq) > interp_gap_smaller_than]
                for seq_kpt in sequences[n]
            ]
            for n in range(nb_persons_to_detect)
        ]
