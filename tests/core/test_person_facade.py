...
def test_assign_source_track_for_segment_preserves_outside_keyframes(
    ...
    mock_person_data_view, mock_marker_data, mock_dal, mock_skeleton
):
    ...
    start_frame = 20
    end_frame = 30
    mock_find_next_stitch_frame.return_value = end_frame

    # Create specific mock instances for source and target MarkerData
    mock_source_md_instance = MagicMock(spec_set=MarkerData) # Use spec_set for better mocking
    mock_target_md_instance = MagicMock(spec_set=MarkerData)
    ...
    # Mock actions for the specific MarkerData instances
    mock_source_action = MagicMock()
    mock_source_md_instance.action = mock_source_action

    mock_target_action = MagicMock()
    mock_target_md_instance.action = mock_target_action
    ...
    mock_dal.set_custom_property.assert_called_once_with(
        mock_ds_obj, mock_dal.ACTIVE_TRACK_INDEX, 1
    )
...
@patch("pose_editor.core.person_facade.dal", autospec=True)
@patch("pose_editor.core.person_facade.MarkerData")
@patch("pose_editor.core.person_facade.PersonDataView", autospec=True)
@patch("pose_editor.core.person_facade.RealPersonInstanceFacade._get_dataseries_for_view")
@patch("pose_editor.core.person_facade.RealPersonInstanceFacade.find_next_stitch_frame") # New patch
def test_assign_source_track_for_segment_preserves_outside_keyframes(
    mock_find_next_stitch_frame, # New mock argument
    mock_get_dataseries_for_view,
    mock_person_data_view, mock_marker_data, mock_dal, mock_skeleton
):
    """Tests that assign_source_track_for_segment correctly replaces keyframes
